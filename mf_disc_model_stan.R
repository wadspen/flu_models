...
int<lower=0> hosp[N];                          // all weekly hospitalizations
...
int<lower=0> m;                                 // points to predict
...
//real<lower=0> upsilons[n_seasons];
row_vector[n_params] zeta;
corr_matrix[n_params] omega;
real gamma[m];
real<lower=0> alpha[n_seasons, m];
...
for (i in 1:n_seasons) {
                          ...
                          //upsilons[i] ~ exponential(upsilon);
}

gamma[m] ~ normal(0,1/tau);
alpha[1,1] ~ normal(0,1/xi);
alpha[2,1] ~ normal(0,1/xi);

for (i in 1:(m-1)) gamma[i] ~ normal(gamma[i+1],1/taumu);

for (i in 2:m) alpha[1,i] ~ normal(alpha[1,i-1],1/ximu);
for (i in 2:m) alpha[2,i] ~ normal(alpha[2,i-1],1/ximu);

for (i in 1:N) {
                  
                  
                  ili[i] ~ normal(asg(theta_s[seasons[i],],x[i]) + gamma[x[i]],
                          epsilons[seasons[i]]);
                          
                  hosp[i] ~ neg_binomial_2(alpha[seasons[i],x[i]]*ili[i],
                            upsilon);
                          
                  //hosp[i] ~ normal(alpha[seasons[i], x[i]]*ili[i],
                  //          upsilon);
                  
                  //hosp[i] ~ lognormal(log((alpha[x[i]] + ili[i])^2)/sqrt(upsilon + (alpha[x[i]] + ili[i])^2),
                  //                    sqrt(log(upsilon/((alpha[x[i]] + ili[i])^2) + 1)));
...
generated quantities {
    ...
    for (i in 1:4) {
      ...
      hosp_pred[i] = neg_binomial_2_rng(alpha[ps,i]*ili_pred[i],upsilon);
      //hosp_pred[i] = normal_rng(alpha[ps, last_week + i]*ili_pred[i],
      //      upsilon);
      //hosp_pred[i] = lognormal_rng(log((alpha[i] + ili[i])^2)/sqrt(upsilon + (alpha[i] + ili[i])^2),
      //                                sqrt(log(upsilon/((alpha[i] + ili[i])^2) + 1)));
    }
}
"